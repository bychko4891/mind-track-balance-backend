...
// import { User, AdminUser } from './users'; // твої ентіті

// 1) Завантажуємо відповідний .env
const envFile =
...
const must = (name: string) => {
  ...
  if (!v) throw new Error(`[auth-service ormconfig] ENV ${name} is missing (from ${envFile})`);
  return v;
};

const host = must('DB_HOST');
const port = Number(must('DB_PORT'));
const username = must('DB_USERNAME_AUTH');
const password = must('DB_PASSWORD_AUTH');
const database = must('DB_NAME_AUTH');
...
export default new DataSource({
  ...
  migrationsTableName: 'typeorm_migrations_auth_users',
  synchronize: false,
});
